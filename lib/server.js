...
const ADDIN_REALM = (process.env.ADDIN_REALM || '').trim();
...
if (['online', 'fba', 'tmg', 'adfs', 'addin'].indexOf(AUTH_TYPE) === -1) {
...
async function getSharePointAuth(rId, res, username, password) {
    const options = {
        ...
        ...(AUTH_TYPE === 'addin' ? {
            ...
            realm: ADDIN_REALM || undefined,
        } : {}),
        ...(['online', 'fba', 'tmg', 'adfs'].includes(AUTH_TYPE) ? {
            username,
            password,
        } : {}),
        ...(AUTH_TYPE === 'adfs' ? {
    ...
    try {
        ...
        if (log.t.enabled) {
            ...
            } else if (maskedAuthRes.headers && maskedAuthRes.headers.Authorization) {
                maskedAuthRes.headers = Object.assign({}, authRes.headers);
                maskedAuthRes.headers.Authorization = maskHeaderValue('authorization', maskedAuthRes.headers.Authorization);
            }
            log.t(`[${rId}] SharePoint authentication response: %o`, authRes);
...
const server = http.createServer(async (req, res) => {
    ...
    if (authRes.headers.Authorization) {
        proxyReqHeaders.authorization = authRes.headers.Authorization;
    }
